...
def _parse_events_xml() -> Dict[str, Dict]:
    """Parse events.xml - retorna dicionário vazio se arquivo não existir"""
    # Verificar se DAYZ_EVENTS_FILE existe no config (pode não existir no servidor de monitoramento)
    if not hasattr(config, 'DAYZ_EVENTS_FILE'):
        return {}
    ...
    try:
        ...
        # Se houver qualquer erro ao parsear, retornar dicionário vazio
        return {}


def _parse_types_xml() -> Dict[str, Dict]:
    """Lê types.xml e extrai lifetime para cada tipo de veículo - retorna dicionário vazio se arquivo não existir"""
    # Verificar se DAYZ_TYPES_FILE existe no config (pode não existir no servidor de monitoramento)
    if not hasattr(config, 'DAYZ_TYPES_FILE'):
        return {}
...
def _load_types_cache(force: bool = False) -> Dict[str, Dict]:
    ...
    # Verificar se DAYZ_TYPES_FILE existe no config
    if not hasattr(config, 'DAYZ_TYPES_FILE'):
        return {}
...
def _load_events_cache(force: bool = False) -> Dict[str, Dict]:
    """Carrega cache do events.xml com verificação de mtime"""
    global _event_cache, _event_cache_mtime

    # Verificar se DAYZ_EVENTS_FILE existe no config
    if not hasattr(config, 'DAYZ_EVENTS_FILE'):
        return {}
...
def get_event_info_for_type(vehicle_type: str) -> Optional[Dict]:
    """Retorna informações do evento para um tipo de veículo"""
...
def get_vehicle_limits_summary(
    ...
    """Retorna resumo de limites de veículos - funciona mesmo sem arquivos XML"""
    ...
    for vehicle_type in target_types:
        ...
        if not event_info:
            ...
            summary[vehicle_type] = {
                ...
                'has_limit': False,
                'blocked': False,
                'warning': 'Tipo não encontrado no events.xml (arquivo pode não estar disponível no servidor de monitoramento)',
            }
            continue
...
def can_spawn_vehicle(
    ...
    """Verifica se um veículo pode ser spawnado"""
