...
handle_dismantled_upper_frame() {
    local line="$1"
    local content="$2"

    local PlayerName PlayerId Position Tool
    ...
    Tool=$(echo "$content" | sed -n 's/.*with \(.*\)$/\1/p' | xargs)
    ...
    local ToolInfo Message
    if [[ -n "$Tool" ]]; then
        ToolInfo=" usando $Tool"
    else
        ToolInfo=""
    fi
    Message="Desmontagem detectada: $SafePlayerInfo removeu o frame superior de uma fence em $Position$ToolInfo"
    ...
    if [[ -n "$PlayerId" ]] && [[ ${#PlayerId} -eq 44 ]] && [[ -n "$Position" ]]; then
        ...
        DetailsJson="{\"fence_type\": \"upper_frame\", \"position\": \"$Position\", \"action\": \"dismantled\", \"tool\": \"$Tool\"}"
