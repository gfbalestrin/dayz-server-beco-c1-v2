$(document).ready(function() {
    ...
    let listContainerRefreshStatus = {};
    let listContainerRefreshRequests = {};
    const QUICK_FILTERS = {
        last10m: { minutes: 10 },
        last1h: { minutes: 60 },
        last4h: { minutes: 240 },
    ...
    // Estado do banner de containers destruídos
    let destroyedContainersAlertClosed = false;
    
    // Função para atualizar contador de containers
    function updateContainersCount(recordsTotal, recordsFiltered) {
        ...
        if (filtered > 0 || total > 0) {
            ...
            countText += ' container' + ((filtered || total) !== 1 ? 's' : '');
        } else {
            countText = 'Nenhum container';
        }
        
        $('#containersCount').text(countText);
    }
    ...
    function updateDestroyedContainersCount(vehiclesData) {
        ...
        const countElement = $('#destroyedContainersCount');
        const alertElement = $('#destroyedContainersAlert');
        
        countElement.text(destroyedCount.toLocaleString('pt-BR'));
        
        // Mostrar/esconder banner baseado na contagem e estado de fechado
        if (destroyedCount > 0 && !destroyedContainersAlertClosed) {
            alertElement.show();
        } else {
            alertElement.hide();
        }
        ...
        containersTable = $('#containersTable').DataTable({
            ...
            ajax: {
                ...
                dataSrc: function(json) {
                    ...
                    if (json && json.recordsTotal !== undefined) {
                        updateContainersCount(json.recordsTotal, json.recordsFiltered);
                    }
                    // Atualizar contador de containers destruídos
                    if (json && json.data) {
                        updateDestroyedContainersCount(json.data);
                    }
                    return json.data;
                }
            },
            columns: [
                ...
                },
                {
                    ...
                    render: function(data) {
                        const containerId = encodeURIComponent(data.ContainerId || '');
                        const mapUrl = `/map?container_id=${containerId}`;
                        return `<a class="btn btn-link p-0" href="${mapUrl}" title="Abrir container no mapa">` +
                ...
                },
                {
                    ...
                    orderable: false,
                    render: function(data) {
                        const containerId = escapeHtml(data);
                        const safeId = containerId;
                        return '' +
                            '<div class="btn-group" role="group">' +
                                '<button class="btn btn-sm btn-secondary me-1 list-refresh-container-btn" ' +
                                    'id="containerListRefreshBtn_' + safeId + '" ' +
                                    'data-container-id="' + containerId + '">' +
                                    '<i class="fas fa-sync-alt me-1"></i>Atualizar' +
                                '</button>' +
                                '<button class="btn btn-sm btn-primary view-history-btn" data-container-id="' + containerId + '">' +
            ...
            // Ordenar por Última Atualização (coluna 5) e,
            ...
            drawCallback: function(settings) {
                // Atualizar contador de containers (fallback caso dataSrc não tenha atualizado)
                try {
                    const api = this.api();
                    const pageInfo = api.page.info();
                    updateContainersCount(pageInfo.recordsTotal, pageInfo.recordsFiltered);
                    
                    // Atualizar contador de containers destruídos (fallback)
                    const data = api.rows({ page: 'current' }).data().toArray();
                    updateDestroyedContainersCount(data);
                } catch (e) {
                    console.error('Erro ao atualizar contador de containers:', e);
                }
            }
        });
    }
    ...
    // ==== Funções de atualização completa de container (checkcontainer) ====
    
    function generateContainerRefreshRequestId() {
        return 'cont_req_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
    }
    
    function setListContainerRefreshState(containerId, isRefreshing) {
        if (!listContainerRefreshStatus) {
            listContainerRefreshStatus = {};
        }
        
        if (isRefreshing) {
            listContainerRefreshStatus[containerId] = true;
        } else {
            delete listContainerRefreshStatus[containerId];
        }
        
        const button = document.getElementById(`containerListRefreshBtn_${containerId}`);
        if (button) {
            if (isRefreshing) {
    ...
    function saveContainerCheckToDatabaseFromList(vehicleId, commandData) {
        ...
        const saveData = {
            container_name: commandData.container_name || 'Container',
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                const errorMsg = error.message || error.error || 'Erro ao salvar dados do container no banco';
        ...
        $.get(`/api/commands/results/${requestId}`)
            ...
            .fail(function(xhr) {
                ...
                if (attempt < 5) {
                    ...
                    const error = xhr.responseJSON || {};
                    const errorMsg = error.message || error.error || 'Erro ao consultar resultado da atualização do container.';
                    if (typeof showToast === 'function') {
                        showToast('Erro', errorMsg, 'error');
                    }
                    setListContainerRefreshState(vehicleId, false);
    ...
    function updateQuickShortcutButtons() {
        $('.container-quick-filter').removeClass('active');
        if (activeQuickFilter) {
            $(`.container-quick-filter[data-filter="${activeQuickFilter}"]`).addClass('active');
        }
    }
    ...
    function reloadTable(resetPage = true) {
        if (containersTable) {
            ...
            destroyedContainersAlertClosed = false;
            ...
            if (resetPage) {
                containersTable.page('first');
            }
            containersTable.ajax.reload(null, false);
        }
    }
    ...
    $('.container-quick-filter').on('click', function() {
    ...
    // Abrir histórico automaticamente se container_id vier na URL
    (function initContainerFromUrl() {
        try {
            const urlParams = new URLSearchParams(window.location.search);
            const containerIdFromUrl = urlParams.get('container_id');
            if (containerIdFromUrl) {
                ...
                const checkReadyInterval = setInterval(function() {
                    if (containersTable) {
                        ...
                        showContainerHistory(containerIdFromUrl);
                    }
                }, 200);
            ...
            console.warn('Erro ao processar container_id da URL:', e);
        }
    })();
    ...
    $(document).on('click', '.view-history-btn', function() {
        ...
        showContainerHistory(containerId);
    });
    
    // Event listener para botão de atualização completa (checkcontainer) na lista
    $(document).on('click', '.list-refresh-container-btn', function() {
        const containerId = $(this).data('container-id');
        refreshContainerFromList(containerId);
    });
    
    // Mostrar histórico do container
    function showContainerHistory(containerId) {
    ...
    function loadHistoryPage(page) {
        ...
        $('#historyLoading').show();
        $('#historyContent').hide();
        $('#historyModalLabel').html('<i class="fas fa-history me-2"></i>Histórico: ' + escapeHtml(currentContainerId));
        ...
        $.ajax({
            url: '/api/containers/' + encodeURIComponent(currentContainerId) + '/history?' + params.toString(),
            type: 'GET',
                        ...
                        <div class="flex-grow-1">
                            <div class="card">
                                <div class="card-header d-flex justify-content-between align-items-center">
                                    <h6 class="mb-0">
                                        ...
                                        ${record.IsPartialUpdate == 1 ? 
                                            '<span class="badge bg-info ms-2" title="Update parcial (apenas posição)">Parcial</span>' : 
                                            ''}
                                    </h6>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="row mt-3">
                                        <div class="col-12">
                                            <h6><i class="fas fa-box me-2"></i>Items</h6>
                                            ${renderItemsWithChanges(record, nextRecord, history, index)}
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    function hasSignificantChanges(prev, curr) {
        ...
        return statusChanged || posChanged || itemsChanged;
    }
    ...
    // Usado tanto em renderItems quanto em renderItemsWithChanges
    ...
    // Renderizar items com indicadores de mudança
    // prev = registro atual sendo renderizado (mais recente na timeline)
    ...
    function renderItemsWithChanges(prev, curr, history, currentIndex) {
        // Se não há próximo registro (curr é null), mostrar apenas o registro atual sem comparação
        if (!curr) {
            return renderItems(prev);
        }
        ...
        // Verificar se há mudanças em items detectadas
        ...
        if (prevIsPartial) {
            ...
            if (history && currentIndex !== undefined && currentIndex !== null) {
                ...
                if (lastComplete) {
                    ...
                    sourceInfo = ' Os items exibidos abaixo são do snapshot completo de <strong>' + sourceTimestamp + '</strong> e foram preservados porque este é um registro parcial.';
                }
            }
            html += '<div class="alert alert-info mb-2">' +
                ...
                '<small>' +
                'Os items mostrados abaixo são do último snapshot completo anterior e não refletem mudanças neste momento específico.' +
                sourceInfo +
                '</small></div>' +
                '</div>';
        }
        
        // Se há mudanças em items detectadas mas um registro é parcial, informar
        if (itemsChangesDetected && (prevIsPartial || currIsPartial)) {
            html += '<div class="alert alert-warning mb-2">' +
                '<i class="fas fa-exclamation-triangle me-2"></i>' +
                '<small>Mudanças em items foram detectadas, mas não podem ser comparadas porque um dos registros é parcial.</small>' +
                '</div>';
        }
        ...
        if (!prevIsPartial && !currIsPartial) {
            ...
            // Se apenas prev é completo, mostrar items do registro completo
            // Isso é especialmente importante quando há mudanças detectadas mas curr é parcial
            if (itemsChangesDetected) {
                html += '<div class="alert alert-warning mb-2">' +
                    '<i class="fas fa-info-circle me-2"></i>' +
                    '<small>Comparação detalhada não disponível (registro anterior é parcial). Exibindo items do snapshot completo atual.</small>' +
                    '</div>';
            }
            html += renderItems(prev);
        } else if (prevIsPartial) {
            // Se o registro atual é parcial, mostrar items herdados com aviso adicional
            ...
            if (prev.items && prev.items.length > 0) {
                html += '<div class="mb-2">' +
                    '<small class="text-muted"><i class="fas fa-info-circle me-1"></i>Os items abaixo foram herdados do último snapshot completo anterior' + lastCompleteInfo + '.</small>' +
                    '</div>';
            }
            html += renderItems(prev);
        } else if (itemsChangesDetected) {
            // Se ambos são parciais mas há mudanças detectadas, mostrar aviso adicional
            html += '<div class="alert alert-warning mb-2">' +
                '<i class="fas fa-exclamation-triangle me-2"></i>' +
                '<small>Mudanças em items foram detectadas, mas ambos os registros são parciais. Não é possível exibir comparação detalhada.</small>' +
                '</div>';
        }
        
        if (!html) {
            html = '<span class="text-muted">Nenhum item</span>';
        }
    ...
    function renderItems(record) {
        ...
        if (!hasItems) {
            return '<span class="text-muted">Nenhum item</span>';
        }
    ...
    // Event listener para fechar banner de containers destruídos
    $('#destroyedContainersAlert').on('close.bs.alert', function() {
        destroyedContainersAlertClosed = true;
    });
