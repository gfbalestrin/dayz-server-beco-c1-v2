...
bool ExecuteCommand(TStringArray tokens)
{
    ...
    switch (command)
    {
        ...
        case "healold":
            ...
            break;
        case "heal":
            // Vida / sangue / choque / energia / água
            target.SetHealth("", "", 100);
            target.SetHealth("GlobalHealth", "Blood", 5000);
            target.SetHealth("GlobalHealth", "Shock", 5000);
            target.GetStatEnergy().Set(4000);
            ...
            // 1) Remove cortes/sangramentos
            if (target.GetBleedingManagerServer())
                target.GetBleedingManagerServer().RemoveAllSources(); // :contentReference[oaicite:3]{index=3}
        
            // 2) Remove doenças (agents)
            target.RemoveAllAgents(); // :contentReference[oaicite:4]{index=4}
        
            // 3) Remove modifiers (febre, wound infection, etc.)
            ModifiersManager mm = target.GetModifiersManager();
            if (mm)
                mm.DeactivateAllModifiers(); // :contentReference[oaicite:5]{index=5}
...
class AdminStaminaService
{
    ...
    void AdminStaminaService()
    {
        m_Timers = new map<string, ref Timer>();
    }
    ...
    void RefreshAdminEpinephrine(PlayerBase player)
    {
        ...
        if (mm.IsModifierActive(eModifiers.MDF_EPINEPHRINE))
            mm.DeactivateModifier(eModifiers.MDF_EPINEPHRINE);

        mm.ActivateModifier(eModifiers.MDF_EPINEPHRINE);
    }

    void Enable(PlayerBase player)
    {
        ...
        // NOTE: aqui "this" é válido porque estamos dentro da classe
    ...
    void Disable(PlayerBase player)
    {
        if (!player || !player.GetIdentity()) return;
        ...
        ModifiersManager mm = player.GetModifiersManager();
        if (mm && mm.IsModifierActive(eModifiers.MDF_EPINEPHRINE))
            mm.DeactivateModifier(eModifiers.MDF_EPINEPHRINE);

        player.MessageStatus("Stamina infinita (admin) desativada");
    }

    bool IsEnabled(PlayerBase player)
    {
